...
public class EgovWebSocketMessengerController {
	 ...
	 * @param 바로접속웹소켓
	 * @param  바로 메시지 가능
	 * @param  관리자 일반으로 구분해야함
	 * @param  세션를 이름이나 아이디로 구분해야함
	 * @return 세션리스트로 1:1 쪽지도 가능하게 만들어야함
	 * 
	 */
	
	
	@RequestMapping(value="/cop/msg/info_msg/msg_main.do")
	public String websocketMsgMain(HttpSession session, ModelMap model) {
		model.addAttribute("loginVO", session.getAttribute("loginVO"));
		return "egovframework/com/ext/msg/info_msg/msg_main";
	}

	@RequestMapping(value="/cop/msg/info_msg/msg_client.do")
	public String websocketMsgClient(HttpSession session, ModelMap model) {
		model.addAttribute("loginVO", session.getAttribute("loginVO"));
		return "egovframework/com/ext/msg/info_msg/msg_client";
	}
