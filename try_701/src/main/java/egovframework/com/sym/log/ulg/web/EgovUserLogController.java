...
public class EgovUserLogController {
	...
	//@IncludedInfo(name="사용로그관리", listUrl= "/sym/log/ulg/SelectUserLogList_nexacro.do", order = 1040 ,gid = 60)
	@RequestMapping(value="/sym/log/ulg/SelectUserLogList_nexacro.do")
	public NexacroResult selectUserLogInf_Nexacro(@ParamDataSet(name = "searchVO") UserLog userLog,
			ModelMap model) throws Exception{
		...
		nexacroResult.addDataSet("resultCnt", _map.get("resultCnt"));
		nexacroResult.addDataSet("resultList",_map.get("resultList"));
	...
	//파라미터 수가 많으면 VO를 생성해서 처리하는 것이 좋다고 생각된다
	@RequestMapping(value="/sym/log/ulg/SelectUserLogDetail_nexacro.do")
	public NexacroResult selectUserLog_nexacro(@ParamDataSet(name= "searchVO") UserLog userLog,
			@ParamVariable(name="occrrncDe") String occrrncDe,
			@ParamVariable(name="rqesterId") String rqesterId,
			@ParamVariable(name="srvcNm") String srvcNm,
			@ParamVariable(name="methodNm") String methodNm,
			ModelMap model) throws Exception{
